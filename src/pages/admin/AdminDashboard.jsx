...
const ShortcutButton = ({ to, icon: Icon, label }) => (
  <Link
    to={to}
    className="flex items-center gap-3 px-4 py-3 glass rounded-2xl text-sm font-semibold text-gray-700 hover:bg-white/40 transition-colors"
  >
...
const AdminDashboard = () => {
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-2xl font-bold text-gray-800 mb-1">Sirimon Hikers Admin</h1>
      ...
      <h2 className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-3">Overview</h2>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        <StatCard
          label="Completed Hikes"
          value={completedHikes?.length ?? 0}
          note={completedHikes?.length === 1 ? 'and counting.' : completedHikes?.length > 1 ? 'and counting.' : 'None yet.'}
        />
      ...
      <h2 className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-3">Quick Actions</h2>
